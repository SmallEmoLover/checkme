...
from .constants import (
    DB_PASSWORD_ROOT, DB_USER_ROOT, BASH_COMMAND_IN_CONTAINER, PATH_TASK_FILES_IN_CONTAINER, RESULT_DIR_LOCAL_NAME,
    PATH_RESULT_IN_CONTAINER)
    ...
    def check(self, abs_path_task_files: str, to_be_checked_file_name: str, verifying_file_name: str) -> bool:
        ...
        self.container.exec_run(
            BASH_COMMAND_IN_CONTAINER.format(f'\
                ...
                < "{to_be_checked_file_name}" > "{PATH_RESULT_IN_CONTAINER}/{TO_BE_CHECKED_RES_FILE_NAME}"\
            ')
        )
        ...
        self.container.exec_run(
            BASH_COMMAND_IN_CONTAINER.format(f'\
                cd "{PATH_TASK_FILES_IN_CONTAINER}"\
                && {MYSQL_CLIENT_ROOT_CONNECTION}\
                < "{verifying_file_name}" > "{PATH_RESULT_IN_CONTAINER}/{VERIFYING_RES_FILE_NAME}"\
            ')
        )

        # проверим различия файлов по содержимому
        unique_cont_key: str = "".join(self.container.name.split("-")[1:])
        path_res_dir: str = os.path.join(abs_path_task_files, RESULT_DIR_LOCAL_NAME.format(unique_cont_key))
        to_be_checked_res_file: str = os.path.join(path_res_dir, TO_BE_CHECKED_RES_FILE_NAME)
        verifying_res_file: str = os.path.join(path_res_dir, VERIFYING_RES_FILE_NAME)
